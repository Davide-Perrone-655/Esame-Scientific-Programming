...
if opts['mod']:
    ...
    for x in x_axis:
        ...
        if flag and opts['take_storie'] and (fmt[opts['unitx']] in os.listdir(os.curdir)):#importante ordine
            flag = False
            while True:
    ...
    if opts['path']:
        grf.func_save(opts['L'], opts['extfield'], opts['unitx'], x_axis, d_oss, opts['out_file'], opts['path'])
    ...
    try:
        datas=salva.read_data(opts['out_file'])
    ...
    for oss in datas['d_oss'].keys():
        grf.plot_graph(datas['x_axis'], datas['d_oss'][oss]['valore'], datas['d_oss'][oss]['errore'], datas['L'] , datas['extfield'], datas['unitx'], oss)
